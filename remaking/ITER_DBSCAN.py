...
try:
    from sentenceEmbedding_pytorch import SentenceEmbedding
except:
    from remaking.sentenceEmbedding_pytorch import SentenceEmbedding
import re
...
class ITER_DBSCAN(DBSCAN):
    ...
    def compute(self, data):
        ...
        if type(data[0]) is str:
            ...
            if self.features is not 'precomputed' or self.embedding_model is None:
                self.embedding_model = SentenceEmbedding()
                data = self.embedding_model.getEmbeddings(data)
        ...
        for i in range(self.max_iteration):
            ...
            cluster_labels = DBSCAN(eps=self.initial_distance, min_samples=self.initial_minimum_samples,
                                    metric='cosine')
            labels = cluster_labels.fit_predict(features)
